...
export async function callVeklomChat(params: {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Veklom request failed with HTTP ${response.status}: ${await readError(response)}`);
    }

    const data = (await response.json()) as VeklomChatResponse;
    return {
      text: contentFromResponse(data),
      model: data.model,
      requestId: data.id,
    };
  } catch (error) {
    if (error instanceof DOMException && error.name === "AbortError") {
      throw new Error(`Veklom request timed out after ${config.timeoutMs}ms.`);
    }
    throw error;
  } finally {
...
export async function checkVeklomHealth(config: VeklomClientConfig = {}): Promise<VeklomHealth> {
  ...
  try {
    ...
    return {
      reachable: false,
      error: error instanceof DOMException && error.name === "AbortError" ? "Veklom health check timed out." : (error as Error).message,
    };
  } finally {
    clearTimeout(timeout);
  }
